...
void write_courses_offered(std::vector<Course> &all_courses) {
  ...
  std::vector<Course> remaining_last;
  for(const auto & x:all_courses){
    if(x.quarter!="null") {
      ofs<<x.title<<','<<x.number_of_units<<','<<x.quarter<<'\n';
    }else{
      remaining_last.push_back(x);
    }
  }
  all_courses=remaining_last;
...
void write_courses_not_offered(std::vector<Course> &unlisted_courses) {
  ...
  ofs<<"Title,Number of Units,Quarter"<<'\n';
  for(const auto & x:unlisted_courses){
